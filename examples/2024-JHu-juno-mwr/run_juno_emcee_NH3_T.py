...
def ln_likelihood(theta, observations, COVALL):
    ...
    simulations = set_atmos_run_RT(nh3, temperature,1.0)  # Use your forward operator here
    residuals = observations - simulations
...
def ln_prior(theta):
    ...
    # RHmax_mean = 1.0  # Mean value for temperature
    # RHmax_stddev = 0.5  # Standard deviation for temperature   
    ...
    if (0 < nh3 < 1000) and (100 < temp < 200): # and (0 <= RHmax <= 1):
        return ln_prior_nh3 + ln_prior_temp #+ln_prior_rhmax
...
if __name__ == "__main__":
    ...
    # Initialize walkers
    n_walkers = 8
    n_dimensions = 2 # nh3, temperature, rh_max_NH3
    initial_guess = [200.0, 150.0] #, 0.5]  # Initial guess for NH3 and temperature
    initial_guesses = [
    ...
    # backend
    filename = f"run_mcmc_NH3_T_background.h5"
