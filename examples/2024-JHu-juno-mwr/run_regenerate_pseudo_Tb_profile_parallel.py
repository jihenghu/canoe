...
def set_atmos_run_RT_concurrent(theta, tbs_shm_name, SH_NH3_shm_name, RH_NH3_shm_name,SH_H2O_shm_name, RH_H2O_shm_name, nstep, n_walkers):
    ...
    for i in range(len(aircolumn)):
        ...
        SH_NH3[int(istep), jindex, i] = ap_mole.hydro()[iNH3]
        SH_H2O[int(istep), jindex, i] = ap_mole.hydro()[iH2O]
...
h5 = h5py.File('run_juno_emcee_pseudo_H2O_T_RHmax_parallel_step_10000.h5', 'r')
...
profOUT = h5py.File(f'run_juno_emcee_H2O_rk1_adapt_H2O_NH3_mixingratio_10000step_last{nstep}.h5', 'w')
